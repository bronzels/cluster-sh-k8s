...
MYCONFDIR=/opt/hdpallcp/conf
MYMNTDIR=/opt/hdpallcp/mnt
if [ -d "${MYMNTDIR}"  ] ;
then
  cp ${MYMNTDIR}/* ${MYCONFDIR}/
  chmod a+rw ${MYCONFDIR}/*
  chmod a+x ${MYCONFDIR}/*.sh
fi
...
docker images|grep hdpallcp
docker images|grep hdpallcp|awk '{print $3}'|xargs docker rmi -f
ansible slave -m shell -a"docker images|grep hdpallcp|awk '{print \$3}'|xargs docker rmi -f"
docker images|grep hdpallcp
