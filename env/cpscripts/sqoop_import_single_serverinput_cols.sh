...
#for (( i=0; i < num_tables; i++ )); do
	...
	sqoop import --connect \"$server\" --username $user --password $pswd --table $name -m 6 --columns \"$cols\" --hcatalog-database default --hcatalog-table $prefix$name --create-hcatalog-table --hcatalog-storage-stanza "stored as orcfile"
	end_tm_tbl=`date +%s%N`;
