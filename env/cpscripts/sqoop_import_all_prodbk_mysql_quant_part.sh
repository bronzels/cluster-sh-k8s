...
for (( i=0; i < num_tables; i++ )); do
	...
	sqoop import --connect "jdbc:mysql://rm-wz905j9253s6l04l6.mysql.rds.aliyuncs.com/"$thisdb"" --username liuxiangbin --password m1njooUE04vc --table $name --columns \"$cols\" -m 6 --hcatalog-database default --hcatalog-table $thisname --create-hcatalog-table --hcatalog-storage-stanza "stored as orcfile"
	end_tm_tbl=`date +%s%N`;
